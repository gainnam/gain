...
var replyList = function() {
	...
	$.ajax({ //$.getJSON 으로 대체 해도 됩니다.
		type:"post",
		url:"/reply/reply_list/${boardVO.bno}/"+page,//113게시물번호에 대한 댓글목록을 가져오는 URL
		dataType:"json",//받을때 json데이터를 받는다.
		success:function(result) {//result에는 댓글 목록을 json데이터로 받음.
			//alert("디버그" + result);
			if(typeof result=="undefined" || result=="" || result==null) {
				$("#div_reply").empty();//조회된 값이 없을 때, 화면clear
				alert('조회된 값이 없습니다.');
			}else{
	...
	$("#deleteReplyBtn").on("click", function(){
		...
		$.ajax({
			...
			success: function(result) {
				if(result=="success") {
					alert("삭제되었습니다.");
					//$("#reply_list").html("");//기존에 있던 reply_list(댓글영역) html디자인 지우기(DB하고는 상관x)
	...
	$("#insertReplyBtn").on("click", function() {//댓글등록버튼을 클릭했을 때 구현내용(아래)
		...
		$.ajax({//통신프로그램
			...
			success:function(result) {//응답이 성공하면(상태값200)위경로에서 반환받은 result(json데이터)를 이용해서 화면을 재구현
				...
				/*
				result = [
					//{rno:댓글번호,bno:게시물번호,reply_text:"첫번째 댓글",replyer:"admin",regdate:타임스탬프}
