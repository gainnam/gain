						...
						<div class="card-body">
							...
							<c:forEach var="index" begin="0" end="1">
								<c:if test="${boardVO.save_file_names[index] != null}">
									...
									<p class="text-muted">
										<a
											href="/download?save_file_name=${boardVO.save_file_names[index]}&real_file_name=${boardVO.real_file_names[index]}">
										...
										<!-- length결과는 2, 2-1=1이 배열의 인덱스 값1 -->
										<!-- 첨부파일이 이미지인지 아닌지 비교해서 img태그를 사용할 지의 여부에 대해 결정(아래) -->
										<!--  fn:contains함수({'jpg','gif','png',...}비교내용, jpg첨부파일 확장자) -->
										<c:choose>
											<c:when
												test="${fn:containsIgnoreCase(CheckImgArray,extName)}">
												<img style="width: 100%;"
													src="/download?save_file_name=${boardVO.save_file_names[index]}&real_file_name=${boardVO.real_file_names[index]}">
											</c:when>
											<c:otherwise>
												<c:out value="${checkImgArray}"></c:out>
												<!-- 사용자 홈페이지 메인에서 최근 게시물 미리보기 이미지가 없을 떄 사용할 예정 -->
											</c:otherwise>
										</c:choose>
										<!-- 결과값 true/false -->
									</p>
