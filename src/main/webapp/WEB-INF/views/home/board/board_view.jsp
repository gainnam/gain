	...
	<div id="container">
		...
		<div class="bodytext_area box_inner">			
			<ul class="bbsview_list">
				...
				<li class="bbs_date">조회수 : <span>${board.view_count}</span></li>
				...
				<c:if test="${boardVO.save_file_names[index] != null}">
					<br>
					<a href="/download?save_file_name=${boardVO.save_file_names[index]}&real_file_name=${boardVO.real_file_names[index]}">${boardVO.real_file_names[index]}파일 다운로드 링크[${index}]</a>
...
<!-- 댓글 페이징 재구현 Representation하는 함수(아래) -->
<script>
var printPageVO = function(pageVO, target) {
	 ...
	 if(pageVO.next){
		 paging = paging + '<li class="paginate_button page-item next" id="example2_next"><a href="'+(pageVO.startPage+1)+'" aria-controls="example2" data-dt-idx="7" tabindex="0" class="page-link">Next</a></li>'
	 }
	 target.html(paging);
}
</script>
...
var replyList = function() {
	...
	$.ajax({
		url:"/reply/reply_list/${boardVO.bno}/"+page,/* 쿼리스트링으로 안보냄. 패스베리어블로 보냄 */
		type:"post",//원랜 get인데, post로 보낼 수 있음.
		dataType:"json",
...
$(document).ready(function(){
	$(".pageVO").on("click", "li a", function(event){
		event.preventDefault();//링크이동 방지(기본값유지)/디폴트 액션(링크이동) 방지
		var page = $(this).attr("href");//get
		$("#reply_page").val(page);//set/val()괄호 안 매개변수가 없으면 get.
		//alert("디버그" + page);
		replyList();
	});
});
</script>
<!-- 댓글 리스트 버튼 클릭 이벤트 처리(아래) -->
<script>
$(document).ready(function(){
...
$(document).ready(function() {
	$("#insertReplyBtn").on("click", function() {//댓글등록버튼을 클릭했을 때 구현내용(아래)
		if("${session_enabled}" == "") {//버튼 클릭 시 비 로그인 시 로그인 화면으로 유도
			alert("회원만 댓글 등록이 가능합니다");
			location.href="/login";//자바스크립트 내장기능(href:hyperTextReference:URL이동함수)
			return false;
		}
		//alert("디버그");
		//Ajax를 이용해서, 화면을 Representation (REST-API방식) 부분 화면을 재구현(아래)
		var bno = "${boardVO.bno}";
		var reply_text = $("#reply_text").val();//input타입은 val()함수로 입력값을 가져올 수 있음.
		var replyer = $("#replyer").val();
		if(reply_text=="" || replyer=="") {
			alert("댓글 내용, 작성자는 필수 입력 사항입니다");
			return false;
		}
		$.ajax({//통신프로그램: J.쿼리에서 내장도니 ㅠ함수ajax({}); 비동기통신
		...
		//여기서부터는 프론트 엔드 개발자 영역
			...
			data:JSON.stringify({//RestAPI서버로 보내는 JSON값
				bno:bno,
				reply_text:reply_text,
				replyer:replyer
			}),
