...
<style>
   	...
   	.length_limit{
	    ...
	    white-space: nowrap;/*줄바꿈할 때, 공백은 포함 안시키는 의미*/
       	}
</style>
<script>
			...
			<div class="about_box">
				...
				<c:forEach var="boardVO" items="${board_list}" varStatus="status">
					...
					<li class="opacity_hover" ><a href="/home/board/board_view?bno=${boardVO.bno}&page=1">
							...
							<c:if test="${save_file_names[status.index] ne ''}"><!-- not equal -->
								<img src="/image_preview?save_file_name=${save_file_names[status.index]}" style="opacity=0.8;"></img>
							</c:if>
							</div>
							<h3 class="length_limit"><c:out value="${boardVO.title}" /></h3>
