...
public class BoardServiceImpl implements IF_BoardService {
	...
	//아래 메서드는 구현이 2개있는데, 조회수 업데이트 정상 처리 후 조회 에러 발생 시, 
	//업데이트한 조회수를 원상복귀(-1)(롤백)처리시키는 작업이 필요.transaction 
	@Transactional//에러발생 시 롤백이 됩니당.
	@Override
	public BoardVO readBoard(Integer bno) throws Exception {
		// bno번호에 해당하는 게시물 조회쿼리 DAO연결 + 해당게시물 조회 카운트 업데이트
	...
	public void insertBoard(BoardVO boardVO) throws Exception {
		...
		for(String save_file_name:save_file_names) {//첨부파일이 1개일대는 한 번만 반복됩니다.
