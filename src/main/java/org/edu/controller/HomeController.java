...
public class HomeController {
	...
	/**
	 * 외부(웹브라우저)루트 / 라는 이름으로 요청 받으면, home.jsp파일에 화면출력(렌더링)이 됩니다.(아래)
	...
	public String contact_send() {
		//데이터 전송 후에 페이지 이동이 필요합니다. 새로고침을 방지하기 위해서.
	...
	public String index() {//메서드(함수)의 리턴(반환값 출력)형태지정 메서드명(매개변수) {구현내용} String index(){}
			//상단 리퀘스트매핑에서 지정한 경로로 접근을 하게되면, 아래있는 index()매서드를 실행해라.
			//매서드명 제일 앞에있는 public은 접근권한에 대한 키워드, public(공공)
			//, private(내부-현재 HomeController클래스에서만 접근가능)
			// public String index() {} 매서드 기본형태
			// $(document).ready(fuction(){}); j쿼리 기본형태
			//리턴반환값으로 사용한 String클래스는 문자열을 반환. 상대경로가 필요합니다.
			//스츠링 MVC프로젝트에서 상대경로의 최상위는 views폴더가 최상위 root폴더입니다.
