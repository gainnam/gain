...
/**
 * ReplyController.java 클래스 댓글 구현 Rest-API전용 컨트롤러=RestAPI서버
 * 
 * @author 김일국
 *
 */
...
public class ReplyController {
	...
	public ResponseEntity<Map<String, Object>> reply_list(@PathVariable("bno") Integer bno) {
		System.out.println("디버그: 패스 베리어블 변수는" + bno );
		ResponseEntity<Map<String, Object>> result = null;

		//ReplyVO replyVO = new ReplyVO();나중에~~~한다고함
		...
		// -----------------------------------------------
		//dummyMapList대신 DB tbl_reply테이블에서 조회된 결과값으로 대체
	...
	// 위 메서드의 반환값 ResponseEntity는 json텍스트(전송내용, 전송상태값)로서 Ajax로 호출한 jsp로 리턴값을 보내게
	// 됩니다.
}
