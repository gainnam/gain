...
public class CommonController {
	...
	/**
	 * 게시물 첨부파일 이미지보기 메서드 구현(스프링 시큐리티 적용 후 IE에서 지원 가능)
	...
	public ResponseEntity<byte[]> getImageAsByteArray(@RequestParam("save_file_name") String save_file_name, HttpServletResponse response) throws IOException {
		...
		int readCount = 0;
		byte[] buffer = new byte[1024];//1KB단위로 읽어들임
		byte[] fileArray = null;
	while((readCount = fis.read(buffer)) != -1){
		baos.write(buffer,0,readCount);
	}
	fileArray = baos.toByteArray();//바이트 단위로 돼있는 변수에 아웃풋스트림내용을 저장해서 return으로 반환
	fis.close();//고전 자바프로그램에서는 메모리 관리를 위해서 fis파일인풋스트림 생성 후 소멸시키는 작업이 필수
	baos.close();//스프링프레임워크 기반의 프로그램구조에서는 close와 같은 메모리관리가 할 필요없습니다. 스프링에garbagecollect기능이 내장
	...
	case "jpeg":
		headers.setContentType(MediaType.IMAGE_JPEG);break;
	case "bmp":headers.setContentType(MediaType.parseMediaType("image/bmp"));break;
	default:
	}
	...
	public String fileUpload(MultipartFile file) throws IOException {
		...
		return saveFileName;//copy로 업로드 이휴에 저장된 real_file_name 스트링 문자열 값 1개를  반환합니다.
	}
